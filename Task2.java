...
class CreateAccount {
    ...
    public CreateAccount(HashMap<String, ArrayList<Integer>> map){
        System.out.println("Enter Account No:");
        Scanner scanner = new Scanner(System.in);
        this.acnt_no = scanner.nextLine();
...
class Transfer {
    ...
    public Transfer(HashMap<String,ArrayList<Integer>> map){
        System.out.println("Enter your account number");
        Scanner scanner = new Scanner(System.in);
        this.acnt_no1 = scanner.nextLine();
...
class Withdraw {
    ...
    int pin;
    int amount;
    public Withdraw(HashMap<String, ArrayList<Integer>> map){
        System.out.println("Enter your account number");
        Scanner scanner = new Scanner(System.in);
        this.acnt_no = scanner.nextLine();
...
class Deposit {
    String acnt_no;
    Integer amount;
    public Deposit(HashMap<String, ArrayList<Integer>> map){
        System.out.println("Enter your Account Number:");
        Scanner scanner = new Scanner(System.in);
        this.acnt_no = scanner.nextLine();
        if (map.containsKey(acnt_no)){
...
class Checkbalance{
    ...
    String acnt_no;
    int pin;
    public Checkbalance(HashMap<String,ArrayList<Integer>> map){
...
public class Task2{ 
    ...
    public static void main(String[] args) {
        ...
        while(true){//boolean to run the infinite loop...
            ...
            switch (choice) {
                case 1:
                    CreateAccount cod_obj=new CreateAccount(map);// passing map as an argument for the class
                    pay_obj.pay(cod_obj);// passing a object to the pay()- polymorphism..
                    break;
                case 2:
                    Deposit dep_obj=new Deposit(map);
                    pay_obj.pay(dep_obj);
                    break;
                case 3:
                    Withdraw with_obj = new Withdraw(map);
                    pay_obj.pay(with_obj);
                    break;
                case 4:
                    Transfer tr_obj=new Transfer(map);
                    pay_obj.pay(tr_obj);
                    break;
                case 5:
                    Checkbalance bal_obj = new Checkbalance(map);
                    pay_obj.pay(bal_obj);
                    break;
                default:
